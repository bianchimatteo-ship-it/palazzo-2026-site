import { fullDate, formatDate } from '../core/time.js?v=20260924-6';
import { DATA_SOURCES, isSelectableParty } from '../data/schema.js?v=20260924-6';
import { isRealCollectionLoaded, loadRealCollections, pristineRecord, realDatabase, refreshAdminOverrides } from '../data/repositories/real-data.js?v=20260924-6';
import { addRoleOverride, clearAdminArchive, exportAdminArchive, hasAdminPin, importAdminArchive, isAdminUnlocked, lockAdmin, removeRoleOverride, resetRecordOverride, saveRecordOverride, setAdminPin, setRecordField, unlockAdmin } from '../data/repositories/admin-store.js?v=20260924-6';
import { renderAdminPanel } from './admin-panel.js?v=20260924-6';
import { deleteLocalLogo, exportLogoConfiguration, getLocalLogo, importLogoConfiguration, listLocalLogos, saveLocalLogo, validateLogoFile } from '../data/repositories/logo-store.js?v=20260924-6';
import { renderPartyArchive, renderPartyProfile } from './party-archive.js?v=20260924-6';
import { renderPoliticianArchive, renderPoliticianProfile } from './politician-archive.js?v=20260924-6';
import { renderLogoAdmin } from './logo-admin.js?v=20260924-6';
import { makeCareerDraft, renderCareerWizard } from './career-wizard.js?v=20260924-6';
import { validateCareerStep } from '../core/career-rules.js?v=20260924-6';
import { renderCampaignPage } from './campaign-mode.js?v=20260924-6';
import { renderParliamentPage } from './parliament-mode.js?v=20260924-6';
import { ELECTION_MODELS } from '../data/simulation/campaign-rules.js?v=20260924-6';
import { CAREER_LEVELS, careerLevelLabel } from '../data/regions.js?v=20260924-6';
import { renderElectionCalendar, renderHeadquarters, renderInbox, renderPartyPosition } from './game-mode.js?v=20260924-6';
import { attachChartInteractions, renderPollsPage } from './polls-mode.js?v=20260924-6';
import { glyph, officeIcon } from './visuals.js?v=20260924-6';
...
const pages = {
  ...
  sondaggi: { title: 'Sondaggi', eyebrow: 'UMORE DEL PAESE', intro: 'Consensi, trend e mondo politico della simulazione.' },
...
export function mountApp(root, store) {
    ...
    root.innerHTML = `
      <div class="app-shell" style="--party-accent:${esc(accent)}">
        ...
        <main class="main-area">
          ...
          <div class="page-wrap">${wizard ? '' : (state.ui.activePage === 'panoramica' ? renderHeadquarters(state, { partyName: party ? partyName(party) : null }) : subpage(state, current, player, party, eventList, catalog, { logoFor, parties:realParties(), selectable:selectableParties(), findParty, realLeader, admin, adminContext }))}</div>
        </main>
  ...
  const collectionsForPage = page => page === 'partiti-lista'
    ...
    : ['parlamento','governo','leggi'].includes(page) ? ['parliamentaryGroups','groupMemberships','chambers','politicians']
  ...
  const ensurePageData = async (page, force = false) => {
    ...
    if (!missing.length && !force) {
      ...
      if (parliamentaryPage) store.initializeParliament(realDatabase.parliamentaryGroups ?? []);
      return;
    }
    try {
  ...
  root.addEventListener('click', async event => {
    ...
    if (wizard && (wizardAction || goto)) {
      ...
      if (wizardAction === 'cancel') wizard = null;
        ...
        else { try { store.createCareer(wizard,realParties(),realDatabase.parliamentaryGroups ?? []); wizard = null; } catch (error) { wizard.errors = [error.message || 'Impossibile creare la carriera.']; } }
      }
  ...
  root.addEventListener('submit', async event => {
    ...
    if (form.matches('[data-law-proposal-form]')) {
      ...
      try { store.proposeLaw({ title: data.get('title'), category: data.get('category'), summary: data.get('summary') }); }
      catch (error) { store.getState().ui.toast = error.message; render(store.getState(),store.getLastSaved()); }
...
function subpage(state, page, player, party, events, catalog, options = {}) {
  ...
  const listContents = {
    ...
    sondaggi: renderPollsPage(state, { logoFor: id => { const record = options.findParty?.(id); return record ? options.logoFor?.(record) : null; }, realLeader: options.realLeader }),
  ...
  const routeGroups = { partito: [['partiti-lista', 'Tutti i partiti']], parlamento: [['governo', 'Governo'], ['leggi', 'Leggi']], governo: [['parlamento', 'Camere'], ['leggi', 'Leggi']], leggi: [['parlamento', 'Camere'], ['governo', 'Governo']], calendario: [['eventi', 'Eventi']], eventi: [['calendario', 'Agenda']], profilo: [['politici', 'Archivio politici']], politici: [['profilo', 'Il tuo profilo']], 'partiti-lista': [['partito', 'Il tuo partito']] };
