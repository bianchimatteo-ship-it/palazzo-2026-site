...
import { SETTINGS_SCHEMA } from '../core/settings.js?v=20260925-3';
import { STAT_LABELS } from '../data/simulation/career-rules.js?v=20260925-3';
...
import { esc, signed } from './charts.js?v=20260925-3';
...
function accountView(account = {}) {
  ...
  if (!account.user) return `<h2>Account e salvataggi online</h2><p class="section-subtitle">Con un account le tue carriere si salvano online e le ritrovi su qualsiasi dispositivo. Il browser ne conserva comunque una copia per giocare anche senza connessione.</p>
    ...
    </div>${note}${account.unreachable ? '<div class="setting-actions"><button class="secondary-button" data-menu-action="start-local">Il servizio non risponde: inizia senza account</button></div><p class="parliament-note">La partita resta salvata in questo browser; potrai collegarla al tuo account appena il servizio torna raggiungibile.</p>' : ''}`;
...
function welcomeView(account = {}, menu = {}) {
  ...
  const blocked = menu.needAccount ? '<p class="account-message" role="status">Per iniziare una nuova carriera crea il tuo account o accedi: bastano un nome utente e una password.</p>' : '';
  const offline = !account.available ? '<p class="parliament-note">Il servizio account non è raggiungibile da questa copia del gioco: puoi iniziare comunque, la partita resta salvata in questo browser e potrai collegarla a un account più avanti.</p><div class="setting-actions"><button class="primary-button" data-menu-action="start-local">Inizia senza account</button></div>' : '';
  return `<span class="section-kicker">PRIMO AVVIO</span><h2>Prima di iniziare: il tuo account</h2><p class="section-subtitle">Un account personale conserva le tue carriere e le rende disponibili su ogni dispositivo.</p>${blocked}<div class="welcome-points">${points}</div>${account.available ? `<div class="setting-actions welcome-actions"><button class="primary-button" data-menu="account" data-account-mode="register">Crea un account</button><button class="secondary-button" data-menu="account" data-account-mode="login">Ho già un account: accedi</button></div><small class="welcome-note">Non serve un indirizzo email. La password non lascia mai il tuo browser in chiaro e non può essere recuperata: conservala con cura.</small>` : offline}`;
}
