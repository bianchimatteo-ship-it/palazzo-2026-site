import { activityProblem, costProblem, describeChoice, describeEffects, nextPartyRank, objectiveProgress, partyContestScore, situation, upcomingElections } from '../core/career-engine.js?v=20260924-10';
import { activeMinisters, CHAMBERS, parliamentGroupFacts } from '../core/parliament-engine.js?v=20260924-10';
import { ACTIVITY_CATEGORIES, PARTY_RANKS, STAT_LABELS, WEEKLY_ACTIVITIES } from '../data/simulation/career-rules.js?v=20260924-10';
import { careerLevelLabel } from '../data/regions.js?v=20260924-10';
import { formatDate } from '../core/time.js?v=20260924-10';
import { renderBarometerPanel } from './polls-mode.js?v=20260924-10';
import { artTile, CATEGORY_VISUALS, EVENT_ICONS, glyph, officeIcon } from './visuals.js?v=20260924-10';
import { ITALIAN_REGIONS } from '../data/regions.js?v=20260924-10';
import { societyMood } from '../core/society-engine.js?v=20260924-10';
import { financeOutlook } from '../core/finance-engine.js?v=20260924-10';
import { isPartyLeader, organOf } from '../core/organization-engine.js?v=20260924-10';
import { renderCountryCard, renderMediaPanel, renderTerritoryCard } from './society-mode.js?v=20260924-10';
import { renderFinanceCard } from './finance-mode.js?v=20260924-10';
import { renderContactsPanel, renderPartyCard } from './organization-mode.js?v=20260924-10';
const weeks = count => `${count} ${count === 1 ? 'settimana' : 'settimane'}`;
import { stateBadge } from './charts.js?v=20260924-10';
...
function alertsStrip(state, gc) {
  ...
  if (finance && ['crisi', 'rischio'].includes(finance.status)) alerts.push([finance.status, 'wallet', finance.debt ? `Debito aperto: ${euro(finance.debt)}` : `Cassa in esaurimento (${weeks(finance.runway ?? 0)})`, 'finanze']);
  ...
  for (const promise of (game.promises ?? []).filter(item => item.status === 'open')) alerts.push([promise.dueWeek - game.week.index <= 3 ? 'rischio' : 'stabile', 'target', `Promessa in ${promise.region}: verifica tra ${weeks(Math.max(0, promise.dueWeek - game.week.index))}`, 'territori']);
  ...
  if (election) alerts.push([election.status === 'open' ? 'crescita' : 'stabile', 'ballot', election.status === 'open' ? `Candidature aperte: ${election.label}` : `${election.label}: candidature tra ${weeks(weeksUntil(state.clock.currentDate, election.windowOpensAt))}`, 'elezioni']);
  const congress = party?.org && !party.org.founder ? party.org.congress.nextWeek - game.week.index : null;
  if (congress !== null && congress >= 0 && congress <= 3) alerts.push(['stabile', 'crown', `Congresso del partito tra ${weeks(congress)}`, 'partito']);
