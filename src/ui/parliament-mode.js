...
function governmentView(parliament, politicians = [], approval = null, extras = {}) {
  ...
  const currentSection = open ? '<section class="government-current"><div class="home-section-heading"><div><span class="section-kicker">' + (led ? 'GOVERNO IN CARICA · SIMULAZIONE' : 'COALIZIONE SIMULATA') + '</span><h2>' + esc(government.name) + '</h2></div>' + dataMarker(DATA_SOURCES.SIMULATION) + '</div>' + (led ? referenceGovernmentPanel(parliament, government, extras) : '') + '<p class="government-members">' + government.coalitionGroupIds.map(id => chipName(all.find(group => group.groupId === id))).filter(Boolean).map(name => '<span>' + esc(name) + '</span>').join('') + government.supportingGroupIds.map(id => chipName(all.find(group => group.groupId === id))).filter(Boolean).map(name => '<span>Sostegno esterno · ' + esc(name) + '</span>').join('') + '</p>' + position + '<div class="minister-list">' + ministers + '</div>' + endedLine + appoint + activeControls + confidenceSummary(government) + (canEdit && !led ? '<details class="coalition-edit"><summary>' + (government.status === 'awaiting-confidence' ? 'Modifica i gruppi proposti' : 'Rinegozia la coalizione') + '</summary><div class="coalition-checks">' + groupChecks(parliament, government.coalitionGroupIds, politicians) + '</div><button class="secondary-button" data-parliament-action="revise-government">Aggiorna la coalizione</button></details>' : '') + '</section>' : '';
