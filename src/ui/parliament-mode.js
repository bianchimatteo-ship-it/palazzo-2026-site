import { activeMinisters, canManageParliament, CHAMBERS, CONTEST_COST, CONTEST_WINDOW_DAYS, LAW_CATEGORIES, MINISTERIAL_PORTFOLIOS, nextParliamentaryRole, parliamentGroupFacts, playerInMajority } from '../core/parliament-engine.js?v=20260924-16';
import { DATA_SOURCES } from '../data/schema.js?v=20260924-16';
import { artTile, glyph, LAW_ICONS } from './visuals.js?v=20260924-16';
import { projectLaw } from '../core/society-engine.js?v=20260924-16';
...
function governmentView(parliament, politicians = [], approval = null) {
  ...
  // Palazzo Chigi's agenda: only a Prime Minister sets it; every six weeks a measure on it reaches the country.
  const premier = open && ['active', 'crisis'].includes(government.status) && government.primeMinister === 'player';
  const agenda = government?.agenda ?? [];
  const agendaSection = premier ? '<section class="government-current government-agenda"><div class="home-section-heading"><div><span class="section-kicker">PRESIDENTE DEL CONSIGLIO · SIMULAZIONE</span><h2>L’agenda del governo</h2></div>' + dataMarker(DATA_SOURCES.SIMULATION) + '</div><p class="parliament-note">Scegli fino a tre priorità (1 giorno). Ogni sei settimane il Consiglio dei ministri approva un provvedimento sulla priorità successiva: costa margine di bilancio, arriva sui territori e cambia l’umore dei cittadini che ne beneficiano o lo subiscono.' + (agenda.length ? ' Provvedimenti già adottati: ' + whole(government.measureCount ?? 0) + '.' : '') + '</p><form class="agenda-form" data-government-agenda-form><div class="agenda-options">' + LAW_CATEGORIES.map(category => '<label class="agenda-option"><input type="checkbox" name="agenda" value="' + esc(category) + '"' + (agenda.includes(category) ? ' checked' : '') + ' /><span>' + esc(category) + '</span></label>').join('') + '</div><button class="primary-button" type="submit">Fissa l’agenda · 1 giorno</button></form></section>' : '';
...
function lawCard(law, parliament, society = null) {
  ...
  const projection = open && society ? projectLaw(society, { category: law.category, compromiseLevel: law.compromiseLevel ?? 0 }) : null;
