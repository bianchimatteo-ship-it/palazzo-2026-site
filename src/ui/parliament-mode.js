import { activeMinisters, canManageParliament, CHAMBERS, CONTEST_COST, CONTEST_WINDOW_DAYS, GOVERNMENT_POST_REQUIREMENTS, governmentPostProblems, MINISTERIAL_PORTFOLIOS, nextParliamentaryRole, parliamentGroupFacts, playerInMajority } from '../core/parliament-engine.js?v=20260924-22';
import { DATA_SOURCES } from '../data/schema.js?v=20260924-22';
import { artTile, glyph, LAW_ICONS } from './visuals.js?v=20260924-22';
import { measureDesign, projectLaw } from '../core/society-engine.js?v=20260924-22';
import { governmentDesk, lawContent, policyFields, policyPreview } from './policy-mode.js?v=20260924-22';
import { SEGMENTS } from '../data/simulation/society-rules.js?v=20260924-22';
...
function governmentView(parliament, politicians = [], approval = null, extras = {}) {
  ...
  const currentSection = open ? '<section class="government-current"><div class="home-section-heading"><div><span class="section-kicker">' + (led ? 'GOVERNO IN CARICA · SIMULAZIONE' : 'COALIZIONE SIMULATA') + '</span><h2>' + esc(government.name) + '</h2></div>' + dataMarker(DATA_SOURCES.SIMULATION) + '</div>' + (led ? referenceGovernmentPanel(parliament, government, extras) : '') + '<p class="government-members">' + government.coalitionGroupIds.map(id => all.find(group => group.groupId === id)?.officialName).filter(Boolean).map(name => '<span>' + esc(name) + '</span>').join('') + government.supportingGroupIds.map(id => all.find(group => group.groupId === id)?.officialName).filter(Boolean).map(name => '<span>Sostegno esterno · ' + esc(name) + '</span>').join('') + '</p>' + position + '<div class="minister-list">' + ministers + '</div>' + endedLine + appoint + activeControls + confidenceSummary(government) + (canEdit && !led ? '<details class="coalition-edit"><summary>' + (government.status === 'awaiting-confidence' ? 'Modifica i gruppi proposti' : 'Rinegozia la coalizione') + '</summary><div class="coalition-checks">' + groupChecks(parliament, government.coalitionGroupIds, politicians) + '</div><button class="secondary-button" data-parliament-action="revise-government">Aggiorna la coalizione</button></details>' : '') + '</section>' : '';
...
export function renderParliamentPage(page, state, options = {}) {
  ...
  if (page === 'governo') {
    ...
    return days + governmentDesk(state) + governmentView(parliament, options.politicians ?? [], state.world?.polls?.at(-1)?.government?.approval ?? null, { stats, secretary: options.secretary, currentDate: state.clock?.currentDate });
  }
