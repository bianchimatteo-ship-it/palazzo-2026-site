...
export function renderPartyProfile(id, logoFor = () => null) {
  ...
  const linked = (realDatabase.politicians ?? []).filter(person => person.partyId === id);
  ...
  const twoPerThousand = (realDatabase.twoPerThousand ?? []).find(item => item.partyId === id && item.source === 'real' && item.verified === true);
